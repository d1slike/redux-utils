...
const generateDefaultReducer = (actionsCount, initialState) => {
    const script =
        `(state = ${JSON.stringify(initialState)}, action) => {` + 'switch(action.type) {';
    const switches = _
        .range(1, actionsCount + 1)
        .map(i => `case "ACTION_${i}": return state;`)
        .join('\n');
...
const generateObjectReducer = (actionsCount, initialState) => {
    ...
    _.range(1, actionsCount + 1).forEach(i => reducer.when(`ACTION_${i}`, state => state));
